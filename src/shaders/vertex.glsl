...
// Perlin noise, variação periodica
float pnoise(vec3 P, vec3 rep)
{
	vec3 Pi0 = mod(floor(P), rep); // Parte Inteiro%periodo
	vec3 Pi1 = mod(Pi0 + vec3(1.0), rep); // Parte (Inteiro+1)%periodo
	Pi0 = mod289(Pi0);
	Pi1 = mod289(Pi1);
	vec3 Pf0 = fract(P); // Parte fracionária para interpolação
	vec3 Pf1 = Pf0 - vec3(1.0); // Parte fracionária - 1.0
